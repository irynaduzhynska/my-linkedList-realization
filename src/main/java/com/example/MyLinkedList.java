...
public class MyLinkedList<T> implements MyLinkedListInterface<T> {
    ...
    public boolean add(T value) {
        if (size == 0) {
            first = last = new Node<T>(null, value, null);
        } else {
            Node<T> currentNode = new Node<T>(last, value, null);
    ...
    public void add(T value, int index) {
        ...
        Node<T> addNode = new Node<T>(currentNode.prev, value, currentNode);
    ...
    private void addFirstElement(T value) {
        Node<T> node = new Node<T>(null, value, first);
        first.prev = node;
        first = node;
        size++;
    }
